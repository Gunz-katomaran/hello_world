#!/usr/bin/env python
'''
This node is used to get data from joystick and convert it into command velocity
'''
#subscribed topics: /joy
#published topics: /cmd_vel,/emergency
import rospy
...
#estop_pub = rospy.Publisher("emergency", Bool, queue_size=1)
...
def joyCB(msg):
    ...
    if msg.buttons[6] == 1 :
        if msg.axes[2] and msg.axes[1]:
            ...
            m = False
            stop = False
            sign_linear = np.sign (msg.axes[1])
            sign_angular = np.sign (msg.axes[2])
            move_u()
    	elif msg.axes[2] and msg.axes[5]:
            forward = False
            rev = False
            left = False
            right = False
        ...
        elif msg.axes[1] == 1 or msg.axes[5] == 1:
            forward = True
            rev = False
            left = False
            right = False
