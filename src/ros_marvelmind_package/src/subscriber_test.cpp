...
{uint8_t lifeTime, i;
  ...
  posewcs.header.frame_id = "/odom";
  posewcs.header.stamp = ros::Time::now();

  posewcs.pose.pose.position.x = x;
  posewcs.pose.pose.position.y = y;
  posewcs.pose.pose.position.z = z;

  posewcs.pose.pose.orientation.x = 0.0;
  posewcs.pose.pose.orientation.y = 0.0;
  posewcs.pose.pose.orientation.z = 0.0;
  posewcs.pose.pose.orientation.w = 1.0;
...
int main(int argc, char **argv)
{
  ...
  rviz_marker_pub = rosNode.advertise<visualization_msgs::Marker>("viz_marker", 1);
  // Set our initial shape type to be a cube
  rviz_shape = visualization_msgs::Marker::CUBE;
  //pose_pub = rosNode.advertise<geometry_msgs::PoseWithCovarianceStamped>("robot_base/pose", 1);
  ros::spin();
