...
#############################################################################
	...
	self.imu=Imu()
        #### parameters #######
	self.pub_odom_tf = bool(rospy.get_param('~publish_odom_tf', True))
	#rospy.loginfo('Parameter %s has value %s', rospy.resolve_name('~publish_odom_tf'), self.pub_odom_tf)
	...
	self.ticks_meter = float(rospy.get_param('ticks_meter', 250047.0432)) / float(rospy.get_param('~linear_scale', 1.0))
	#rospy.loginfo('Parameter %s has value %s', rospy.resolve_name('~linear_scale'), self.ticks_meter)
        #self.base_width = float(rospy.get_param('robot_width', 0.4)) # The wheel base width in meters
	self.base_width = float(rospy.get_param('robot_width', 0.4)) / float(rospy.get_param('~angular_scale', 1.0))
	#rospy.loginfo('Parameter %s has value %s', rospy.resolve_name('~angular_scale'), self.base_width)
    ...
    ############################################################################
		...
		self.x = self.x + ( cos( self.th ) * x - sin( self.th ) * y )
            ...
            if self.publish_tf and self.pub_odom_tf:
