...
from robo_waiter.msg import Tabled
...
path_name1 = res =  table_name = mile_name = kg = home_point = parking_point = goal_status = None
parking = home_pt_b = False

def path_load_cb (req):
    global response, path_name, path_name1, tl_list,tl_xy_list, res
    path_name = req.path_name
    path_name1 = req.table_path_name
    f = open (config_file_path + path_name + ".txt", "r")
    f1 = open (config_file_path + path_name1 + ".txt", "r")
    data = f.read ()
    data1 = f1.read ()
    b = json.loads(data)
    b1 = json.loads(data1)
...
def table_cb (req):
    global kg, table_name, home_pt_b
...
def pose_cb(msg):
    ...
    if parking:
        ...
        if distance <= 0.2:
            print "near_by_parking"
            ft = rospy.ServiceProxy('/parking', Park)
            ft (status=True)
